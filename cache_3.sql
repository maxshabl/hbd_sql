...
with
	bk_days as (
		select 
			count(book_days)::integer as count_bk_days, extract(dow from book_days.book_days::timestamp)::integer as bk_days 
	...
	-- список suplements and discounts
	 sad as (
		( select  'cnsr'::text as sad_name, rest_days, rest_nights, paxes, adults, childes, infants, standard_capacity, max_pax, max_children, 
			...
			file_id, hotel_code, room_type, characteristic, base_board, week_days,
